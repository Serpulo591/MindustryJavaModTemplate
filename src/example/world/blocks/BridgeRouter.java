...
public class BridgeRouter extends Block {
    ...
    public BridgeRouter(String name) {
        ...
        config(Point2.class, (BridgeRouterBuild tile, Point2 i) -> {
            ...
            tile.links.add(pos);
            tile.transportCounters.clear();
            tile.transportCounters.put(pos, 0f);
        });
        ...
        config(Integer.class, (BridgeRouterBuild tile, Integer i) -> {
            ...
            if (i != -1) {
                tile.links.add(i);
                tile.transportCounters.put(i, 0f);
            }
        });
    ...
    public boolean linkValid(Tile tile, Tile other, boolean checkDouble) {
        ...
        return ((other.block() == tile.block() && tile.block() == this) ||
                (!(tile.block() instanceof BridgeRouter) && other.block() == this))
    ...
    public class BridgeRouterBuild extends Building {
        ...
        public IntMap<Float> transportCounters = new IntMap<>();

        @Override
        public void pickedUp() {
            links.clear();
            transportCounters.clear();
        }
        ...
        public boolean onConfigureBuildTapped(Building other) {
            ...
            if (other instanceof BridgeRouterBuild b && linkValid(tile, other.tile)) {
                ...
                if (links.contains(pos)) {
                    ...
                    transportCounters.remove(pos);
                } else {
                    links.add(pos);
                    transportCounters.put(pos, 0f);
                }
                return false;
            }
            return true;
        ...
        public void updateTile() {
            ...
            for (int i = 0; i < links.size; i++) {
                ...
                float counter = transportCounters.get(pos, 0f);
                counter += edelta();
                while (counter >= transportTime) {
                    Item item = items.take();
                    if (item != null && other.build.acceptItem(this, item)) {
                        other.build.handleItem(this, item);
                        moved = true;
                    } else if (item != null) {
                        items.add(item, 1);
                        items.undoFlow(item);
                    }
                    counter -= transportTime;
                }
                transportCounters.put(pos, counter);
            }
        }
