...
public class CoreUnloaderBuild extends Building {
    public int link = -1;                     // 目标核心的打包坐标
    public float transportCounter = 0f;       // 传输计时器
    public boolean hadValidLink;              // 上一帧连接是否有效

    // ----- 连接验证 -----
    public boolean linkValid(Tile other) {
        if (other == null || !other.buildable()) return false;
        ...
        return dx * dx + dy * dy <= range * range; // 球形范围
    }
    ...
    @Override
    public void config(Integer value) {
        link = value != null ? value : -1;
    }
    ...
    public void drawSelect() {
        ...
        for (int dx = -range; dx <= range; dx++) {
            for (int dy = -range; dy <= range; dy++) {
                ...
                if (t != null && t.build instanceof CoreBlock.CoreBuild && t.team == team) {
